...
def create_fusion_datasets(root_directory, output_dir=None, backup_dir=None):
    ...
    Args:
        ...
        backup_dir: Directory to backup existing Fusion files (default: C:\\Users\\XXX\\Desktop\\backups\\OriginalFusionDatasets)
    """
    if backup_dir is None:
        try:
            backup_dir = os.path.join(root_directory, 'backups')
        except Exception:
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        print("\n Default backup directory is root directory to search for CSV files + '\\backups'")
        return
