...
EXCLUDED_FROM_RANKING = {'Fusion', 'CalibrationDataset', 'Yhdistelmäaineisto', 'Kalibrointiaineisto'}
...
def extract_model_and_epoch_info(csv_filepath):
    ...
    if stem.endswith('_detailed_files'):
        ...
        # Remove _results_YYYYMMDD_HHMMSS suffix (ResNet naming convention)
        results_match = re.match(r'^(.+?)_results_\d{8}_\d{6}$', stem)
        if results_match:
            dataset_name = results_match.group(1)
        else:
            dataset_name = stem

    # --- Normalize dataset names across models ---
    # Rename "Authors" / "AuthorsCompiledSounds" to "Authors"
    if dataset_name.lower() in ('Authors', 'merilainencompiledsounds'):
        dataset_name = 'Authors'

    # Handle Wonjun_Yi / WonjunYi case
...
def find_optimal_threshold(df, metric='f1', calibrated_threshold=0.5):
    ...
    lower = max(eps, observed_min - eps)
    upper = min(1.0 - eps, observed_max + eps)
    if not (lower < upper):
    ...
    unique_probs = unique_probs[(unique_probs >= lower) & (unique_probs <= upper)]
...
def create_ranking_table(results, output_dir, suffix=''):
    ...
    for (model_name, epoch_num), datasets in results.items():
        # Filter out excluded datasets for aggregate metrics
        ranking_datasets = {name: metrics for name, metrics in datasets.items()
                            if name not in EXCLUDED_FROM_RANKING}

        if not ranking_datasets:
            print(f"  ⚠ No datasets remaining after excluding {EXCLUDED_FROM_RANKING} for {model_name} Epoch {epoch_num}")
            continue

        # Collect all metrics (only from non-excluded datasets)
        brier_scores = [metrics['brier_score'] for metrics in ranking_datasets.values()]
        f1_scores = [metrics['f1'] for metrics in ranking_datasets.values() if metrics['f1'] is not None]
        all_accuracies = [metrics['accuracy'] for metrics in ranking_datasets.values()]
        precision_scores = [metrics['precision'] for metrics in ranking_datasets.values() if metrics['precision'] is not None]
        recall_scores = [metrics['recall'] for metrics in ranking_datasets.values() if metrics['recall'] is not None]

        # Count dataset types (only non-excluded)
        ...
        # Calculate weighted Brier Score (weighted by sample count, excluding Fusion/Calibration)
        sample_counts = [metrics['total_samples'] for metrics in ranking_datasets.values()]
        ...
        # Calculate weighted F1 (weighted by sample count, only two-class non-excluded datasets)
        weighted_f1 = None
        two_class_sample_counts = [metrics['total_samples'] for metrics in ranking_datasets.values() if metrics['f1'] is not None]
        two_class_total = sum(two_class_sample_counts)
        if f1_scores and two_class_total > 0:
        ...
        # Calculate threshold statistics (non-excluded datasets only)
        thresholds = [metrics['threshold'] for metrics in ranking_datasets.values()]
        ...
        rankings.append({
            ...
            'Two_Class_Samples': two_class_total,
            'Two_Class_Pct': (two_class_total / total_samples * 100) if total_samples > 0 else 0,
    ...
    summary_df = pd.DataFrame([
        {
            ...
            'Two_Class_Samples': r['Two_Class_Samples'],
            'Two_Class_Pct': f"{r['Two_Class_Pct']:.1f}%"
        }
        for r in rankings
    ])
    ...
    for entry in rankings:
        ...
        for dataset_name, metrics in entry['datasets_detail'].items():
            excluded = dataset_name in EXCLUDED_FROM_RANKING
            detailed_rows.append({
                'Rank': rank,
                'Model': model_name,
                'Epoch': epoch_num,
                'Dataset': dataset_name,
                'Excluded_From_Ranking': 'Yes' if excluded else 'No',
    ...
    print(f"Excluded from aggregates: {', '.join(sorted(EXCLUDED_FROM_RANKING))}")
    print(f"{'='*80}\n")
    print(summary_df.to_string(index=False))

    # Warn if two-class datasets represent a very small fraction of total samples
    for r in rankings:
        if r['Two_Class_Pct'] < 5.0 and r['Num_Two_Class'] > 0:
            print(f"\n  ⚠ NOTE: {r['Model']} Epoch {r['Epoch']}: F1/Precision/Recall are based on only "
                  f"{r['Num_Two_Class']} two-class dataset(s) ({r['Two_Class_Samples']} samples, "
                  f"{r['Two_Class_Pct']:.1f}% of total). These metrics may not be representative.")
