...
def plot_embedding_simple(
    ...
    class_labels = {"drone": "Drone", "no drone": "No Drone"}
    
    for label in ["drone", "no drone"]:
        mask = df["label"] == label
        if mask.sum() == 0:
            continue
...
def plot_embedding_by_dataset(
    ...
    # Markers for class
    class_markers = {"drone": "o", "no-drone": "s"}
...
def main():
    ...
    if UMAP_AVAILABLE:
        ...
        plot_embedding_simple(
            ...
            "UMAP: Soundclips by class",
            os.path.join(OUT_DIR, "umap_by_class.png")
        )
        
        plot_embedding_by_dataset(
            coords_umap, df_sampled,
            "UMAP: Soundclips by dataset",
            os.path.join(OUT_DIR, "umap_by_dataset.png")
        )
        
        plot_embedding(
            coords_umap, df_sampled,
            "UMAP: Soundclips by dataset and class",
