...
def plot_combined_det_curves():
    ...
    for idx, (csv_file, epoch, aug) in enumerate(file_infos):
        ...
        if 'true_label' in df.columns and 'drone_probability' in df.columns:
            ...
            print(f"  Error: mandatory columns not found {csv_file}")
            continue
    ...
    ax.set_xlabel('False Positive Ratio (FPR) – probit-scaling', fontsize=12)
    ax.set_ylabel('False Negative Ratio (FNR) – probit-scaling', fontsize=12)
    ax.set_title('DET-plots (wav2vec2, 1 epoch) – augmentation comparison', fontsize=14)

    legend_title = 'Augmentation'
    if any_oob:
        legend_title = legend_title + ' (Plot might be out of bounds)'
    ...
    plt.show()
    print("Plot saved as: 'combined_det_curves.png'")
