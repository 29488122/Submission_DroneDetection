...
# --- CONFIGURATION ---
# Paths are resolved relative to this script's location (the project root).
# No edits should be needed unless you have moved datasets outside Datasets/.

_ROOT = Path(__file__).parent
_DS   = _ROOT / "Datasets"

# A list of all the directories containing your CLEANED evaluation datasets.
# The script will look for 'yes_drone'/'drone' and 'unknown' subfolders inside each.
SOURCE_DATASETS = [
    str(_DS / "Al-Emadi"),
    str(_DS / "AuthorsCompiledSounds"),
    str(_DS / "DronePrint"),
    str(_DS / "EmoSoundscapes"),
    str(_DS / "ESC-50-master"),
    str(_DS / "H-2"),
    str(_DS / "Svanstrom & Englund"),
    str(_DS / "UrbanSound8K"),
    str(_DS / "Yi et al"),
]

# The destination directory where the calibration files will be moved/copied.
# This directory will be created if it doesn't exist.
CALIBRATION_DIR = str(_DS / "eval_threshold")
...
def _compare_to_manifest(
    selections_by_class: Dict[str, List[Tuple[Path, str]]],
    dataset_slug_map: Dict[str, str],
    label: str = "planned",
) -> None:
    manifest_path = Path(__file__).parent / "eval_manifest.csv"
    if not manifest_path.exists():
        print(f"\n[manifest compare] eval_manifest.csv not found at {manifest_path}, skipping.")
        return

    # Load manifest: filename -> class (drone/unknown mapped to yes_drone/unknown)
    ...
    with manifest_path.open(encoding="utf-8") as f:
        for row in _csv_mod.DictReader(f):
            ...
            if fname:
                # normalise class label to match script's "drone"/"unknown"
                manifest[fname] = "drone" if "drone" in cls else "unknown"

    # Build the set of destination filenames the script would produce
    script_files: Dict[str, str] = {}   # dest_filename -> class
    for class_label, selections in selections_by_class.items():
        for f_path, ds_slug in selections:
            dest_name = f"{ds_slug}_{f_path.name}"
            script_files[dest_name] = class_label

    manifest_set = set(manifest.keys())
    script_set   = set(script_files.keys())

    in_both      = manifest_set & script_set
    only_manifest = manifest_set - script_set
    only_script   = script_set - manifest_set

    print(f"\n{'='*60}")
    print(f"  Manifest comparison ({label} vs eval_manifest.csv)")
    print(f"{'='*60}")
    print(f"  eval_manifest.csv entries : {len(manifest_set):>5,}")
    print(f"  {label:25s} files : {len(script_set):>5,}")
    print(f"  Matching                  : {len(in_both):>5,}")
    print(f"  Only in manifest          : {len(only_manifest):>5,}")
    print(f"  Only in {label:<17s}: {len(only_script):>5,}")

    if only_manifest:
        print(f"\n  Files in manifest but NOT in {label} (first 15):")
        for name in sorted(only_manifest)[:15]:
            print(f"    [{manifest[name]}]  {name}")
        if len(only_manifest) > 15:
            print(f"    … and {len(only_manifest)-15} more")

    if only_script:
        print(f"\n  Files in {label} but NOT in manifest (first 15):")
        for name in sorted(only_script)[:15]:
            print(f"    [{script_files[name]}]  {name}")
        if len(only_script) > 15:
            print(f"    … and {len(only_script)-15} more")

    if in_both == manifest_set == script_set:
        print("\n  ✓ Perfect match — selection is identical to eval_manifest.csv")
    print(f"{'='*60}\n")
...
def main():
    ...
    if DRY_RUN:
        ...
        _compare_to_manifest(selections_by_class, dataset_labels, label="planned")
    ...
    _compare_to_manifest(selections_by_class, dataset_labels, label="executed")
