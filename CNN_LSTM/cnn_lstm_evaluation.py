...
class EvaluationConfig:
    """Configuration for CNN-LSTM evaluation. Note that max_file_length is safety measure when we are limited in memory."""
    model_path: str
    data_dir: str
    output_dir: str
    batch_size: int = 8
...
class LargeFileHandler:
    ...
    def aggregate_predictions(self, predictions: List[float], method: str = "mean") -> float:
        ...
        For the research, we use mean, but optional aggregation for testing purposes is provided."""
        if not predictions:
            return 0.0
...
class CNNLSTMEvaluator:
    ...
    Based on dataset structure you might want to change dron_class_index"""
    ...
    def load_model(self):
        ...
        if os.path.isdir(self.config.model_path):
            potential_files = [
                "model.pt",           # saved by SaveAtEpochsCallback / torch.save(state_dict)
                "pytorch_model.bin",
                "model.pth",
                "best_model.pth",
                "final_model.pth",
    ...
    def evaluate_file(self, file_path: str, true_label: int) -> Optional[Dict]:
        ...
        #For some reason we have inverted class labels so this will simply flip the prediction logic around.
        # And we will perform analysis from separate csv files which contains the aggregation probability.
...
def calibrate_threshold(model: nn.Module, config: EvaluationConfig, calibration_data_dir: str = None,
    ...
    """Threshold calibration with INVERTED logic for CNN-LSTM model. Again we deal with inverted class labels.
    """
    ...
    #IN all cases we have more than 10 since we are using calibration dataset. BUT if for some reason we are testing on
    #Separate datasets then this will handle the issue of small sample size with brute force.
    if len(all_probs) < 10:
...
def get_model_paths(model_input: str) -> Dict[str, str]:
    ...
    if os.path.isfile(model_input):
        ...
        for item in os.listdir(model_input):
            ...
            elif item.endswith(('.pth', '.bin', '.safetensors', 'model.pt')):
...
def main():
    ...
    parser.add_argument("--drone_class_index", type=int, choices=[0,1], default=0,
                        help="Column index corresponding to 'yes_drone' class (static). Default 0 for CNN-LSTM models.")
    ...
    if args.output_dir:
        ...
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        base_output_dir = _project_root / "Evaluation" / "CNN_LSTM" / f"cnn_lstm_evaluation_{timestamp}"
