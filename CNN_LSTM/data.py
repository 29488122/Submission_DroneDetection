...
#Validation split only for evaluation on Al-Emadi dataset. Testing is on a separate script.
def load_and_split(
    data_dir: str,
    val_size: float = 0.2,
    test_size: float = 0.0,
    ...
    # IF for some reason we want to change test or val. Split into train and (validation + test)
    if val_size > 0 and test_size > 0:
