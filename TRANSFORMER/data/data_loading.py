...
from TRANSFORMER.utils.audio_processing import (
    ...
    warn_if_label_fell_back_to_unknown,
)
...
def load_and_prepare_clip_based(path: str, label_override: str = None, max_clips_per_split=10000, clip_samples=16000):
    ...
    for chunk_start in range(0, len(raw), CHUNK_SIZE):
        ...
        # Process this chunk
        chunk_input_arrays, chunk_labels, chunk_file_ids, chunk_lengths, chunk_file_paths = process_audio_chunk(
...
def _process_very_large_files(very_large_files, label_override, trainer=None):
    ...
    for file_idx, item, original_length in very_large_files_tuples:
        ...
        source_file_path = audio.get("path", "") or ""
        resolved_label = label_override or extract_label_from_path(source_file_path)
        warn_if_label_fell_back_to_unknown(source_file_path, resolved_label)
        resolved_label_id = LABEL2ID.get(resolved_label, LABEL2ID.get("unknown", 0))
