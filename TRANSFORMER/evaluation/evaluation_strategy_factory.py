...
class EvaluationStrategy(ABC):
    ...
    def __init__(self, model_path, batch_size, output_dir=None):
        self.model_path = model_path
        self.batch_size = batch_size
        if output_dir is None:
            _project_root = pathlib.Path(
                __file__).resolve().parent.parent.parent
            output_dir = str(_project_root / "eval_results")
...
class ClipBasedEvaluationStrategy(EvaluationStrategy):
    ...
    def _collect_file_probs_and_labels(self, dataset_splits, num_files, dataset_name):
        ...
        for split_idx, ds in enumerate(dataset_splits):
            predictions, probabilities, labels, file_ids, _ = self.streamlined_evaluate_dataset_split(
    ...
    def streamlined_evaluate_dataset_split(self, dataset, dataset_name, split_idx=0, total_splits=1):
        ...
        for i in range(0, len(dataset), self.batch_size):
            ...
            with torch.no_grad():
                ...
                # Collect file paths if available in the batch
                if 'file_paths' in batch.column_names:
                    all_file_paths.extend(batch['file_paths'])
                else:
                    all_file_paths.extend([""] * len(batch_preds))

        return all_predictions, all_probabilities, all_true_labels, all_file_ids, all_file_paths
    ...
    def process_dataset(self, dataset_splits, num_files, dataset_name):
        ...
        all_clip_labels = []
        all_file_ids = []
        all_clip_file_paths = []  # per-clip file paths for building the file_id→path map

        cumulative_files = 0
        # Process each split using streamlined approach
        for split_idx, ds in enumerate(dataset_splits):
            predictions, probabilities, labels, file_ids, clip_file_paths = self.streamlined_evaluate_dataset_split(
                ds, dataset_name, split_idx, len(dataset_splits)
            )
            ...
            all_clip_labels.extend(labels)
            all_file_ids.extend(adjusted_file_ids)
            all_clip_file_paths.extend(clip_file_paths)
        ...
        # Build file_id → file_path mapping from clip data (first path seen per file_id wins)
        file_id_to_path = {}
        for fid, fpath in zip(all_file_ids, all_clip_file_paths):
            if fid not in file_id_to_path:
                file_id_to_path[fid] = fpath
        ...
        # Resolve file paths in the same sorted-file-id order used by aggregate_predictions_by_confidence
        unique_file_ids_sorted = sorted(set(all_file_ids))
        file_paths_resolved = [file_id_to_path.get(fid, "") for fid in unique_file_ids_sorted]
        ...
        return {
            ...
            'file_paths': file_paths_resolved,
...
class WholeFileEvaluationStrategy(EvaluationStrategy):
    ...
    def process_dataset(self, dataset_splits, num_files, dataset_name):
        ...
        file_results_dict = {}  # key: original_file_id, value: list of (prediction, probability, label, file_path)
        ...
        for split_idx, ds in enumerate(dataset_splits):
            ...
            for b_i, idxs in enumerate(batches):
                ...
                try:
                    ...
                    # Extract file paths from batch if available
                    try:
                        file_paths_batch = mb["file_paths"] if "file_paths" in mb.column_names else [""] * len(file_predictions)
                    except Exception:
                        file_paths_batch = [""] * len(file_predictions)
                    ...
                    for pred, prob, label, orig_file_id, fpath in zip(file_predictions, drone_probabilities, file_labels,
                                                               original_file_ids, file_paths_batch):
                        if orig_file_id not in file_results_dict:
                            file_results_dict[orig_file_id] = []
                        file_results_dict[orig_file_id].append((pred, prob, label, fpath))
        ...
        for file_id, (original_file_id, results) in enumerate(file_results_dict.items()):
            ...
            preds, probs, labels, fpaths = zip(*results)
            ...
            final_file_path = fpaths[0]  # All parts share the same source file
            ...
            all_file_paths.append(final_file_path)
