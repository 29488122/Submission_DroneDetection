...
def anonymize_path(path: str, keep_parts: int = 3) -> str:
    """Return a tail-only path for logging/reporting to avoid exposing user directories."""
    if not path:
        return ""
    ...
    for ds_name, metrics in results.items():
        if 'probabilities' in metrics and 'true_labels' in metrics:
            ...
            if data_type == "file":
                ...
                file_paths_list = metrics.get('file_paths', [])
                ...
                for i in range(len(true_labels)):
                    ...
                    raw_path = file_paths_list[i] if i < len(file_paths_list) else ""
                    display_path = anonymize_path(raw_path)

                    dataset_results.append({
                        'file_id': i,
                        'file_name': os.path.basename(raw_path) if raw_path else "",
