...
def anonymize_path_for_logs(path: str, keep_parts: int = 3) -> str:
    """Return a tail-only path for logs to avoid exposing absolute user paths."""
    if not path:
        return ""
    ...
    tail = parts[-max(1, keep_parts):]
    return ".../" + "/".join(tail)


def warn_if_label_fell_back_to_unknown(file_path: str, resolved_label: str):
    """Warn when folder-based labeling resolves to unknown from a non-unknown parent."""
    if not file_path:
        return

    parent = Path(file_path).parent.name
    if resolved_label == "unknown" and parent.lower() != "unknown":
        display_path = anonymize_path_for_logs(file_path)
        print(
            f"Warning: label fallback to 'unknown' for file '{display_path}' "
...
def create_chunked_dataset(input_arrays, labels, file_ids, lengths, max_clips_per_split, file_paths=None):
    """
    ...
    Args:
        ...
        file_paths: Optional list of source file paths (one per clip)
    ...
    if file_paths is not None:
        chunk_data["file_paths"] = file_paths
...
def process_audio_chunk(files_chunk, label_override, audio_processor_func, clip_samples=None):
    ...
    Returns:
        Tuple of (input_arrays, labels, file_ids, lengths, file_paths)
    """
    chunk_input_arrays = []
    chunk_labels = []
    chunk_file_ids = []
    ...
    for file_idx, item, original_length in files_chunk:
        audio = item["audio"]
        arr = audio["array"]
        file_path = audio.get("path", "") or ""

        # Use label override if provided, otherwise extract from path
        original_label = label_override or extract_label_from_path(file_path)
        warn_if_label_fell_back_to_unknown(file_path, original_label)
    ...
    return chunk_input_arrays, chunk_labels, chunk_file_ids, chunk_lengths, chunk_file_paths
