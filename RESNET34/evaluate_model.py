...
def calibrate_threshold(model: nn.Module, config: EvaluationConfig, calibration_data_dir: str = None,
    ...
    # Infer which class index corresponds to 'drone' using the calibration set
...
def main():
    ...
    # Create timestamped output directory under project Evaluation/ if not specified
    if args.output_dir is None:
        _project_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        if args.dataset:
            args.output_dir = os.path.join(_project_root, "Evaluation", "RESNET34", f"resnet34_evaluation_{args.dataset}_{timestamp}")
        else:
            args.output_dir = os.path.join(_project_root, "Evaluation", "RESNET34", f"resnet34_evaluation_all_datasets_{timestamp}")
