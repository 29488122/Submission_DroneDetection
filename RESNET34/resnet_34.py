...
class SaveAtEpochsCallback(TrainerCallback):
    """Trigger Trainer's built-in save at specific epoch milestones (mirrors CNN-LSTM approach)."""
    def __init__(self, save_epochs: list):
...
def alt_data_split_for_custom_validation():
    #Make sure to set the data paths for train_set as described in config.py
    dataset = load_dataset("audiofolder", data_dir=TRAIN_SET)
...
def main_loop(epoch: int):
    ...
    # Resolve output directory relative to project root (RESNET34/ -> submission_123/)
    _project_root = Path(__file__).resolve().parent.parent
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    model_dir = _project_root / "Models" / "RESNET34" / f"resnet34_{timestamp}_{epoch}epochs"
    model_dir.mkdir(parents=True, exist_ok=True)

    # Load and split data programmatically (same as CNN-LSTM)
    split_ds = load_and_split(TRAIN_SET, val_size=0.2, seed=seed)
    train_raw = split_ds["train"]
    val_raw   = split_ds["validation"]
    print(f"Loaded {len(train_raw)} train / {len(val_raw)} validation examples")

    ds = {
        "train":      preprocess_split(train_raw),
        "validation": preprocess_split(val_raw),
    }
    ...
    per_device_bs = 16
    # warmup = 10% of total gradient-update steps
    total_steps = (len(ds["train"]) // per_device_bs) * epoch
    ...
    training_args = TrainingArguments(
        ...
        save_strategy="no",         # SaveAtEpochsCallback sets should_save=True at milestone epochs only
        save_total_limit=len(RUNS),
        report_to="none",
        log_level="info",
        dataloader_num_workers=min((os.cpu_count() or 1) - 1, 4),
    ...
    trainer = WeightedTrainer(
        ...
        callbacks=[
            EvalPrinterCallback(),
            SaveAtEpochsCallback(RUNS),
        ],
    )
    trainer.train()

    # Save final model weights via Trainer (produces model.safetensors + config)
...
def main():
    print(f"Training ResNet-34 to {max(RUNS)} epochs, checkpoints at: {RUNS}")
    main_loop(max(RUNS))
